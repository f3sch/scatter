...
struct linear_congruential_permutation_engine {
  ...
  {
    m_x = (a * m_x + c) &
          m_mask; // & m_mask is the same as % m (if m is a power of 2)
    return m_x;
  }
  size_t min()
  {
...
template <typename T>
using Vec =
    std::vector<T, default_init_allocator<T, tbb::cache_aligned_allocator<T> > >;
using DataVec = std::vector<
    DataType,
    default_init_allocator<DataType, tbb::cache_aligned_allocator<DataType> > >;
using IndexVec = std::vector<
    Index, default_init_allocator<Index, tbb::cache_aligned_allocator<Index> > >;
...
auto makeComputedInput(size_t n)
{
  ...
  auto index = ranges::views::generate_n(
      linear_congruential_permutation_engine{ n, 461887 }, n);
...
template <typename DataType = float, typename Index = size_t>
std::pair<Vec<DataType>, Vec<Index> >
makeNormalDistributedShuffle(size_t n, size_t distance, double prob = 1.0)
{
  Vec<DataType> vec(n);
  Vec<Index> index(n);
  std::random_device rnd_device;
  ...
  for (size_t i = 0; i < n; ++i) {
    auto j = static_cast<size_t>(
        (static_cast<std::ptrdiff_t>(shuffle_dist(mersenne_engine)) % n + n) %
        n);
